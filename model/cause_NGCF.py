...
class NGCF_cause(nn.Module):
    ...
    def forward(self, users, pos_items, neg_items, drop_flag=True, test_flag = False):
        ...
        control = helper.compute_2i_regularization_id(pos_items + neg_items, self.n_item // 2)

        control_embed = i_g_embeddings[control, :]
        i_g_embeddings = i_g_embeddings[pos_items + neg_items, :]
        emb_logits = torch.sum(u_g_embeddings_cause * i_g_embeddings, dim=1, keepdim=True)


        if self.cf_distance == "l1":
            #print("Using L1 difference between treatment and control embeddings")
            cf_loss = torch.mean(torch.sum(torch.abs(i_g_embeddings - control_embed), dim=1))

        elif self.cf_distance == "l2":
            #print("Using L2 difference between treatment and control embeddings")
            cf_loss = torch.sqrt(torch.sum((i_g_embeddings / torch.norm(i_g_embeddings, dim=1, keepdim=True) - control_embed / torch.norm(control_embed, dim=1, keepdim=True)) ** 2))

        elif self.cf_distance == "cos":
            #print("Using Cosine difference between treatment and control embeddings")
            cos = nn.CosineSimilarity(dim=1, eps=1e-6)
            cf_loss = 1 - cos(i_g_embeddings / torch.norm(i_g_embeddings, dim=1, keepdim=True), control_embed / torch.norm(control_embed, dim=1, keepdim=True))
        ...
        return all_embeddings, u_g_embeddings[users, :], i_g_embeddings, pos_i_g_embeddings, neg_i_g_embeddings, logits, prediction
