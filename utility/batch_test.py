...
def get_performance(user_pos_test, r, auc, Ks, K_max_item_score):
    ...
    return {'recall': np.array(recall), 'precision': np.array(precision),
            'ndcg': np.array(ndcg), 'hit_ratio': np.array(hit_ratio), 'auc': auc, 'mse': np.array(mse), 'log_loss': np.array(log_loss)}
...
def test(model, users_to_test, drop_flag=False, batch_test_flag=False):
    result = {'precision': np.zeros(len(Ks)), 'recall': np.zeros(len(Ks)), 'ndcg': np.zeros(len(Ks)),
              'hit_ratio': np.zeros(len(Ks)), 'auc': 0., 'mse': 0., 'log_loss': 0.}
    ...
    for u_batch_id in range(1):
        ...
        if batch_test_flag:
            ...
            for i_batch_id in range(n_item_batchs):
                ...
                if drop_flag == False:
                    u_g_embeddings, i_g_embeddings, pos_i_g_embeddings, _, logits, prediction = model(user_batch,
                                                                                                      item_batch,
                                                                                                      [],
                                                                                                      drop_flag=False)
            ...
            if drop_flag == False:
                all_emb = model(user_batch,
                                item_batch,
                                [],
                                drop_flag=False, test_flag=True)
